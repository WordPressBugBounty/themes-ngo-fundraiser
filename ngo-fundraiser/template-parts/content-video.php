...
<?php
  $ngo_fundraiser_content = apply_filters( 'the_content', get_the_content() );
  $ngo_fundraiser_video = false;

  // Only get video from the content if a playlist isn't present.
  if ( false === strpos( $ngo_fundraiser_content, 'wp-playlist-script' ) ) {
    $ngo_fundraiser_video = get_media_embedded_in_content( $ngo_fundraiser_content, array( 'video', 'object', 'embed', 'iframe' ) );
  }
?>
  ...
  <article id="post-<?php the_ID(); ?>" <?php post_class('article-box'); ?>>
    <?php
      if ( ! is_single() ) {
        ...
        if ( ! empty( $ngo_fundraiser_video ) ) {
          foreach ( $ngo_fundraiser_video as $ngo_fundraiser_video_html ) {
            echo '<div class="entry-video">';
              echo $ngo_fundraiser_video_html;
            echo '</div>';
          }
        };
      };
