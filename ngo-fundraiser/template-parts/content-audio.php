...
<?php
  ...
  $ngo_fundraiser_audio = false;

  // Only get audio from the content if a playlist isn't present.
  if ( false === strpos( $ngo_fundraiser_content, 'wp-playlist-script' ) ) {
    $ngo_fundraiser_audio = get_media_embedded_in_content( $ngo_fundraiser_content, array( 'audio' ) );
  }
?>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('article-box'); ?>>
      <?php
        if ( ! is_single() ) {
          ...
          if ( ! empty( $ngo_fundraiser_audio ) ) {
            foreach ( $ngo_fundraiser_audio as $ngo_fundraiser_audio_html ) {
              echo '<div class="entry-audio">';
                echo $ngo_fundraiser_audio_html;
